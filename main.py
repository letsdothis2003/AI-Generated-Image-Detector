#Please make sure all the libraries and requirements are imported(check requirements.txt)
...
def main():
    ...
    # These match the optimized parameters for Hybrid Spatial/Texture Analysis
    TARGET_SIZE = (64, 64) 
    PCA_COMPONENTS = 120 
    SVM_C = 15.0 
    SVM_KERNEL = "rbf"
    ...
    print("--- AI Image Detection System (Hybrid Spatial & Texture Analysis) ---")
    ...
    if X_train.size == 0 or X_test.size == 0:
        print(f"Error: Could not load images at {data_directory}. Please ensure the 'Data' folder exists.")
        return
    ...
    # Includes HOG, LBP (Texture), GLCM (Spatial), and Noise Variance
    print(f"\nExtracting Hybrid Feature Vector (HOG + LBP + GLCM)...")
    ...
    plot_confusion_matrix(
        ...
        class_names=["REAL", "FAKE"], 
        title="Hybrid Spatial Analysis Confusion Matrix", 
        save_path="ai_detection_cm_hybrid.png"
    )

    show_example_predictions(X_test, y_test, results['y_pred'], num_examples=10)

    print("\nOptimization complete. Spatial Domain Results saved.")
